...
- After disk execution finishes, the installer mounts the EFI/root partitions, rewrites `/etc/fstab` with their `blkid` UUIDs, generates a minimal `/boot/grub/grub.cfg`, and runs `grub-install` inside the chroot. This bootloader stage is gated by `installer.write_gate`, `SP_ENABLE_DISK_EXECUTE=1`, and the new `SP_ENABLE_BOOTLOADER=1` toggle (use `SP_SKIP_BOOTLOADER=1` for skips and `SP_DEBUG_BOOTLOADER=1` for extra logging) and emits `[SP-INSTALLER] bootloader START/END` plus the final `[SP-INSTALLER] COMPLETE` marker so downstream tooling can verify the completion sequence.

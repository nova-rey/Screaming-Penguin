...
sp_bootstrap_dev_nodes() {
    sp_log "state=dev-bootstrap" "phase=start"

    mount -t devtmpfs devtmpfs /dev 2>/dev/null || \
        mount -t tmpfs devtmpfs /dev 2>/dev/null || true

    mdev_bin="${SP_BOOTSTRAP_MDEV_BIN:-$(command -v mdev || true)}"
    if [ -n "$mdev_bin" ] && [ -x "$mdev_bin" ]; then
        if "$mdev_bin" -s >/dev/null 2>&1; then
            sp_log "state=dev-bootstrap" "phase=mdev" "result=ok" "cmd=${mdev_bin}"
        else
            sp_log "state=dev-bootstrap" "phase=mdev" "result=failed" "cmd=${mdev_bin}"
        fi
    else
        sp_log "state=dev-bootstrap" "phase=mdev" "result=missing"
    fi

    if [ ! -e /dev/console ] || { [ ! -c /dev/console ] && [ ! -p /dev/console ]; }; then

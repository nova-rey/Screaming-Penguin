...
sp_config_load() {
    ...
    SP_CFG_ROOTFS_PATH="$(yq -r '.installer.rootfs.tarball // .rootfs.path // ""' "$config_path")"
    SP_CFG_HOSTNAME="$(yq -r '.installer.rootfs.hostname // .system.hostname // ""' "$config_path")"
    SP_CFG_TIMEZONE="$(yq -r '.installer.rootfs.timezone // .system.timezone // ""' "$config_path")"
    SP_CFG_LOCALE="$(yq -r '.installer.rootfs.locale // .system.locale // ""' "$config_path")"
    SP_CFG_USER_NAME="$(yq -r '.installer.rootfs.username // .user.name // ""' "$config_path")"
    SP_CFG_USER_PASSWORD_HASH="$(yq -r '.installer.rootfs.password_hash // .user.password_hash // ""' "$config_path")"
